...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:nitin@localhost:5432/factory"
...
def machine(id):
    ...
    with psycopg2.connect(CONNECTION) as conn:
        ...
        query_eff = """SELECT product_id, cycle_time FROM Cycle_Time WHERE machine_id=(%s); """
        cursor = conn.cursor()
        cursor.execute(query_eff,id)
        eff_list = (cursor.fetchall())

        query_simtime = """select time from job_data order by time desc limit 1;"""
        cursor = conn.cursor()
        cursor.execute(query_simtime)
        end = (cursor.fetchall())

        query_simtime = """select time from job_data order by time asc limit 1;"""
        cursor = conn.cursor()
        cursor.execute(query_simtime)
        end = (cursor.fetchall())
        simtime= (end[0]-start[0]).total_seconds()
        pro_eff={}
        for i in eff_list:
            pro_eff[i[0]]=[]
...
def jobs(id):
    ...
    return render_template('job_no.html',job_no=job_no,job_deets=job_deets, id=id)
